...
def lambda_handler(event, context):  # pragma: no cover
    ...
    if config.scale == "out":
        ...
        for region_name in ['us-east-1', 'us-west-2']:
            scale_out_dynamodb(region_name, config.cluster_scaling_configs[config.profile]['dynamodb'])
        ...
        # Scale in DynamoDB
        for region_name in ['us-east-1', 'us-west-2']:
            scale_in_dynamodb(region_name, config.cluster_scaling_configs[config.profile]['dynamodb'])
